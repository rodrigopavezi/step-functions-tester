...
function getPlatform() {
  ...
  if (!allowedPlatforms.includes(platform)) {
    throw new Error(
      `Unsupported platform "${platform}" in the library consider opening a PR to add support`
    );
  }
...
function fixStepFunction(definition) {
  return mapValuesDeep(definition, function (value, key, obj) {
    ...
    if (_.isObject(value) && value.Type === 'Task' && value.Resource) {
      ...
      const functionName = parameters ? parameters.FunctionName : undefined;
      const inputKey = parameters ? 'input' : 'input.$';
      return _.assign(value, {
        Parameters: {
          [samConstants.FUNCTION_KEY]: functionName ? functionName : resource,
          [inputKey]: parameters || '$',
        },
        Resource: constants.LAMBDA_ARN,
      });
    }
